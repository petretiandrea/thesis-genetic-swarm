...
#ifndef SWARM_GEN_MATRIX_H
#define SWARM_GEN_MATRIX_H
...
public:
    ...
    explicit Matrix<T>(std::vector<std::vector<T> > const &array);
    Matrix<T>();
    ...
    Matrix<T> applyFunction(std::function<T(int, int, T)> function) const;
...
template <class T>
Matrix<T> Matrix<T>::applyFunction(std::function<T(int, int, T)> function) const{
    Matrix<T> result(height, width);

    for (int i=0 ; i<height ; i++){
        for (int j=0 ; j<width ; j++){
            result.array[i][j] = function(i, j, array[i][j]);
        }
    }
